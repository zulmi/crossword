(function($){
	$.fn.crossword = function(opts) {	
			...
			var showAnswers=opts.showAnswers || false;//false = don't showAnswer in crossword,true = showAnswer in crossword
			var GAME_DELIM='-';
			...
			/**
			 * Name for savegame cookie.
			 * @type {String}
			 */
//			var cookieName = LOCALSTORAGE_KEY+opts.id;

			/**
			 * Path for cookie to stop it leaking all over the
			...
			var puzInit = {
				...
				*/
				...
				buildHintButton: function(){
					...
					$button.click(function(){
						if(hintsRemaining < 1){
							//alert(puzz.data[activePosition].answer);
							return;
						}
						...
						var clue = puzz.data[activePosition].answer.substr(possibleCells[random],1);
                       	$entry.val(clue);
						
						//crossing out clue if crossword solve with hint                       
						...
						if(gameComplete){
							...
							seconds = c % 60;
							if(c==-1){
								var message = '<p>You completed the crossword, well done!</p>'; //message if crossword comple and not using timer
							}else{
								var message = '<p>you spent '+minutes+' minutes '+seconds+' seconds playing the puzzle.</p>'; //message if crossword comple and using timer	
							}

							popup(message);//display popup dialog//
							clearTimeout(t);/*stop timer when crossword is complete*/
							timer_is_on=0;/*reset timer*/
							c=0;
						}
						var pc=possibleCells.length;
						_this.updateHintsRemaining(--pc);
						_this.saveGame();
					});
				...
				*/
				checkAnswer: function(e) {					
					var valToCheck, currVal, cells,l;
			...
			var util = {
				...
				highlightEntry: function() {
					...
					puzInit.updateHintsRemaining(puzz.data[activePosition].answer.length);
					$actives.eq(0).focus();
					$actives.eq(0).select();
				},
